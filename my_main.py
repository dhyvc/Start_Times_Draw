...
medium_plus_categories = ['H21B', 'H40']
...
Stats_Courses = []
Stats_Requested_Time_Slots = []
Stats_Mean_Deveiation_From_Request = []
Stats_Max_Deviation_From_Request = []
...
def read_start_file(filename, first, last, window_size, blank_slot_interval, event_type, optimization_iterations, consecutive_club_value, consecutive_category_value):
    ...
    if event_type == 'option2':
        ...
        gold_categories = ['D70', 'D75', 'D80', 'H75', 'H80', 'H85', 'H90']
        short_plus_women_categories = ['קצר+', 'קצר פלוס' ,'D21C', 'D40', 'D45', 'D50', 'D55', 'D60', 'D65']
        short_plus_men_categories = ['H50B', 'H60B', 'H65', 'H70']
...
#            print ('course name: ' + category[0][0])
    ...
    print ("The stat for the draw: ")
    print (Stats_Courses)
    
    write_course_stats(Stats_Courses, "./downloads/", event_type)
    for elements in range(len(Stats_Courses)):
        Stats_Courses.pop()
...
def draw_start_times(current_window_index, start_windows, list_of_runners, first_open_slot, blank_slot_counter,
    ...
    Stat_Current_Max = 0.0
    Stat_Current_Total = 0.0
    Stat_Current_Mean = 0.0
    name_of_victim = ""
    
    print("The current window is: ", start_windows[current_window_index])
    print("The course is: ", Stats_Courses[-1][0])
...
#        print ("Type of runner[5] is: ", type(runner[5]))
#        print ("Type of start window is: ", type(start_windows[current_window_index]))
        distance_from_requested_time = abs((datetime.datetime.combine(datetime.date.today(), runner[5]) - datetime.datetime.combine(datetime.date.today(), start_windows[current_window_index]))).seconds/60

    # Collect the statistics of the course regarding distance from requsts
        print ("Runner distance from requested time: ", distance_from_requested_time)
        Stat_Current_Total += distance_from_requested_time
        if distance_from_requested_time > Stat_Current_Max:
            Stat_Current_Max = distance_from_requested_time
            name_of_victim = runner[2]
        print ("Running total minutes from requests: ", Stat_Current_Total)
    
    # Analyze the statistics of the course regarding distance from requsts
    if len(best_list_of_runners) > 0:
        Stat_Current_Mean = round(Stat_Current_Total/len(best_list_of_runners),2)
    else:
        Stat_Current_Mean = 0.0
        
    print ("Mean deviation from requested time is: ", Stat_Current_Mean)
    print ("Max deviation from requested time is: ", Stat_Current_Max, " and the victim name is: ", name_of_victim)
    Stats_Courses[-1][1].append(start_windows[current_window_index])
...
def write_vacant_slots_by_course(input_file, working_dir, first_start, last_start, event_type):
    ...
    forest_categories_row_height = 50
    ...
    if event_type == 'option2':
        ws.row_dimensions[3].height = forest_categories_row_height
    else:
...
def write_course_stats (stats_list, working_dir, event_type):
    """This function writes out the Stats.xlsx file with course statistics information."""
    filename = working_dir + "Stats.csv"
    xlfilename = working_dir + "Stats.xlsx"
#    open(xlfilename, 'w').close()
#    if os.path.exists(xlfilename):
#        os.remove(xlfilename)
    ...
    title_cell.border = thick_red_cell_border_template
    title_cell.alignment = openpyxl.styles.Alignment(horizontal = 'center')
    ws.cell(row = 1, column = 1, value = "סטטיסטיקות על הגרלת הזמנים")
    
    # Set up the additional titles on the page
    ws.cell(row = 2, column = 1).border = thin_black_cell_border_template
    ws.cell(row = 2, column = 1).alignment = openpyxl.styles.Alignment(horizontal = 'center', vertical = 'center')
    ws.cell(row = 2, column = 1, value = "שעת זינוק מבוקשת")
    ...
    number_of_rows = len(stats_list[0][1])
    
    # Fill the start times column
    for index in range (int(number_of_rows)):
        hour = str(stats_list[0][1][index].hour)
        if stats_list[0][1][index].minute == 0:
            minute = "00"
        else:
            minute = str(stats_list[0][1][index].minute)
        ws.cell(row = index + 4, column = 1, value = hour + ":" + minute)
        ws.cell(row = index + 4, column = 1).alignment = openpyxl.styles.Alignment(horizontal = 'center', vertical = 'center')
        ws.cell(row = index + 4, column = 1).border = thin_black_cell_border_template
        
    # Create borders for all the relevant cells in the worksheet
#    for line in range(4, len(input_file)+1):
    for line in range(4, int(number_of_rows) +4):
        for col in range(2, len(course_fields)+2):
            ws.cell(row = line, column = col).border = thin_black_cell_border_template
    
    # Loop over the requeted starting times and fill the cells with the stats
    
#    for index in range (int(number_of_rows) +1):
    for index in range (int(number_of_rows)):
        for course in stats_list:
            if stats_list[stats_list.index(course)][0] != "kids" and stats_list[stats_list.index(course)][0] != "undefined":
                # print ("current course: ", course)
                # print ("current column: ", stats_list.index(course) + 2)
